...
author: "BIOS 11141 - An Introduction to Computational Biology"
output: html_document
---
<span style="color:red"> In all assignments submitted for this course, rather than using scripts, you will be submitting a knitted HTML answer file (**saved as Computer-Assignment-1-#LastnameFirstname.html**) that performs the desired calculations and addresses the given tasks. **Turn in your work via the assignment 1 on Canvas by 2:30 pm CST, a week after the discussion day**.\span 
...
<span style="color:green"> 3.1  Write a line of code that will find the value of pi raised to the 6th power (R has a built in value for pi, called simply `pi`) in the box of R code chunk below (write your answer directly into the RStudio editor window). \span

```{r}
pi^6
```

<span style="color:green">  3.2  Using scientific notation, multiply three million by pi in the following code chunk. \span
...
<span style="color:blue"> R treats TRUE as 1 and FALSE as 0 \span

b.) What are some benefits of R handling these types of expressions as it does? \span

<span style="color:blue"> We may sometimes want to run a trial with 2 outcomes (TRUE/FALSE) and maybe count the number of successful trials. In R, to do this we can just sum all outcomes \span
...
<span style="color:blue"> y is actually storing a value. y is unchanged when we alter x \span

b.) What is the difference between a variable storing an expression and storing a value? \span

<span style="color:blue"> If a variable stores an expression, it will be changed if one of the variables within the expression changes. On the other hand, if a variable just holds a value, it will not change \span
...
<span style="color:blue"> The function c() combines its parameters into a vector and returns it \span
...
a.) Use the subtraction operator `-` to subtract `myVec` and the number `8`. Save this in a new variable so that the original myVec remains the same. <br>
```{r}
newVec <- myVec - 8
newVec
```
...
<span style="color:blue"> This operation subtracts 8 from each element of myVec \span
...
<span style="color:green"> 4.6 What do the following indexing operations do? Explain each. \span <br>
a.) `myVec[3:5]` <br>

<span style="color:blue"> Selects the third through fifth (inclusive) elements from myVec \span

b.) `myVec[-3]` <br>

<span style="color:blue"> Removes the third element from myVec \span

c.) `myVec[c(1,3,8)]` <br>
<span style="color:blue"> Selects the first, third, and eighth element from myVec \span

d.) `2*myVec`  <br>

<span style="color:blue"> Multiplies each element of myVec by 2 \span

e.) `myVec[1] <- 7.5`

<span style="color:blue"> Sets the first element of myVec to 7.5 \span

<span style="color:green"> 4.7 A normal resting heart rate for adults ranges from 60 to 100 beats per minute. <br>
a) Create a vector `heartRate` that spans the range of normal resting heart rates in intervals of 2bpm (use the `c()` function, or check out `?seq()`, which will be seen in part 5). <br>
...
b) The average ejection volume for adults is 69 mL/beat. Store this average ejection volume in a variable `averageEjection`. Using these data, create a vector `ejectionVolumes` containing the total amount of blood (in mL) ejected in *30 minutes* for each heart rate in your range. \span
...
<span style="color:blue"> "!=" means not equals to. ">=" is greater than or equal to. "<=" is less than or equal to. "==" is equals to. "<" is strictly less than. ">" is strictly greater than \span
...
<span style="color:blue"> "||" is the OR operator \span
...
```
<span style="color:blue"> The operation is boolean indexing into the vector multiplesOfFive and selecting the entries that are not divisible by ten \span
...
<span style="color:blue"> If the value stored in carSpeed is greater than that stored in speedLimit, then the code prints "Give ticket", otherwise "Do nothing" \span
...
<span style="color:blue"> The carSpeed is still greater than the speedLimit, so the code still prints out "Give ticket" \span
...
<span style="color:blue"> 1 is interpreted as TRUE, so the code within the if statement runs \span
...
<span style="color:green"> 6.4 Assign a numeric value of your choice to a variable `bodyTemp`. Write an if, else statement that instructs patients to "seek medical attention" if their `bodyTemp` is above 45 degrees or below 40 degrees, or to "do nothing" if their temperature is within an acceptable range. \span
...
<span style="color:blue"> "i" us defined within the for loop. It is implicitly defined at the beginning of the for loop, and then reassigned each iteration of the loop \span
b.) Is it a global variable (hint: look at your environment window)?<br>
<span style="color:blue"> Yes, "i" is a global variable because it shows up in the Global Environment \span
c.) Explain what this loop is doing. <br>
<span style="color:blue"> The loop is printing out the first 10 even numbers \span
d.) If you define a variable `i` outside the for loop prior to its execution, does it affect the calculation? \span
<span style="color:blue"> No, defining "i" outside of the loop before its execution does not affect the calculation \span
...
<span style="color:blue"> If x %% 2 == 0, then x is even. If x %% 2 == 1, then x is odd \span
...
<span style="color:blue"> It would result in an infinite loop because the variable x is never updated so it (x = 3) will always be less than or equal to 6 \span
...
b.) Call this function to calculate the square of 269 and **silently** assign it to a variable `squared`.</span>
```{r}
...
b.) Execute that function on a vector containing values 9 through 30. <br>
```{r}
calcMean(9:30)
```
