---
title: "Computer Assignment 6: Introduction to Unix and Computing on Midway"
author: "BIOS 11141"
output: html_document
---
<span style="color:purple">
**Assignment goals:** <br>
**1. Complete the Center Star algorithm**<br>
**2. Learn basic Unix commands** <br>
**3. Write and run basic Unix bash scripts** <br>
**4. Login and submit jobs to the Midway Supercomputer**
\span

<span style="color:red"> Please upload a knitted HTML file titled "Ans6LastnameFirstname.html" and the Rmd to Canvas by 2:30pm CST the Friday after discussion. \span 

<span style="color:red"> Before we get into learning about Unix and Midway, we will first put together everything we learned in the previous week to complete the Center Star algorithm. Before starting, make sure that you run the functions defined in the last Computer Lab so that they will be present in your environment! \span

To recap what you need for performing Center Star alignment, you need to perform the following steps after you have the sequences and the criteria for performing alignment: 1) Take in the list of sequences and perform pairwise alignment as a handshake problem (you need to use pairAlign). Record the alignment results (the patterns and subjects, not the score) into two separate variables. You can either store them as two matrices or two vectors. 2) Calculate the distances among the sequences based on the alignment and obtain the distance matrix or distance vector (Still, there are two approaches) 3) Calculate the total distances for each sequence. 4) Find the center sequence based on the total distance and the sequence length. 5) Extract the patterns and subjects 6) Perform buildMSA() using patterns,subjects,and center. All these steps need to be included in the centerStar() function.
...
<span style="color:green"> 0.7 For each alignment against the center sequence, retrieve the list of patterns and the list of subjects. Use this data along with the index of the center sequence to run `buildMSA()` on the sequences in `sequences` from 3.3 from Lab5. \span  
...
<span style="color:green"> 1.1 What is the path to your home directory in your computer? <br> \span

<span style="color:blue">
My home directory is C:/Users/icthy
</span>
...
<span style="color:green"> 1.2 What would be the relative path to `HideFiles.sh` be if our present working directory was the `Users` directory? What would the relative path to `HideFiles.sh` be if we were currently in the `funstuff` folder? <br> \span

<span style="color:blue">
If the present directory is Users, the relative path is /smithmj/scripts/funstuff/HideFiles.sh
</span>

<span style="color:blue">
If the present directory is funstuff, the relative path is just HideFiles.sh
</span>
...
<span style="color:green"> 1.3 What are the two periods `..` referencing? <br> \span

<span style="color:blue">
The two periods signify to go back one directory (go to the parent folder of the current folder)
</span>

<span style="color:green"> 1.4 Navigate to your home directory. Now construct a command with `cd` that takes you to your root directory from your home directory. Paste your command into your assignment exercise document. <br> \span

<span style="color:blue">
The command that takes me to the root directory from the home directory is cd ../../../
</span>
...
<span style="color:green"> `ls -l` (that is the letter ‘l’, not the number 1) <br> \span 
...
<span style="color:blue">
Uses a long listing format and sorts by file size (with largest first)
</span>

<span style="color:green"> `ls -lt`  \span 

<span style="color:blue">
Uses a long listing format and sorts by time (with newest first)
</span>

<span style="color:green"> `ls -ltr`  \span

<span style="color:blue">
Uses a long listing format and sorts by time (with newest last)
</span>

<span style="color:green"> `ls -g`  \span  

<span style="color:blue">
Uses a long listing format but does not list the owner.
</span>
...
<span style="color:blue">
The command lists all the folders in the home directory that start with "D"
</span>
...
<span style="color:blue">
The subdirectories of the pcfb folder are examples, sandbox, and scripts. The smallest size file in the examples subdirectory is LatLon.txt
</span>
...
<span style="color:blue">
Assuming initially the home directory is the examples folder, the command to copy the protein file from examples to the pcfb folder is "cp structure_1s36.pdb ../"
</span>

<span style="color:blue">
Assuming the home directory is now the pcfb folder, the command to move the protein file from the pcfb folder to the sandbox folder is "mv structure_1s36.pdb ./sandbox"
</span>
...
<span style="color:blue">
The protein represented is called OBELIN, and it is from the organism OBELIA LONGISSIMA. There are 1493 protein atoms.
</span>
...
<span style="color:blue">
For me, I am allowed to run the myScript.sh file. When I look at the permissions, I see all of r, w, x (I can read, write, and execute)
</span>
...
<span style="color:green"> 2.1 Within your home directory on Midway, create a new subdirectory entitled `CompAsn6` using the `mkdir` command. Navigate to this new folder, and determine its absolute path. Paste these commands below. \span
...
<span style="color:blue">
The main reason is because we are not the only ones using the supercomputer at a given time. We shouldn't be able to disrupt/hijack other people's jobs and other people shouldn't be able to do the same to us. Therefore, we are not able to log on directly to the compute nodes, and instead we submit jobs, and the actual running of jobs is out of our hands.
</span>
...
The last 5 lines, which do not start with `#SBATCH`, contains the commands to execute the R script `CompAsn6source.R`.
...
<span style="color:blue">
I chose the name "homochr2_orf" to specify that we were looking for the ORFs in chromosome 2. 
</span>
...
<span style="color:green"> 2.6 What bash command is executed to run the `CompAsn6source.R` script? \span

<span style="color:blue">
The command is "sbatch job-submit-updated.sh"
</span>

We need to complete one final step to copy the sequence data from the class directory on Midway to our personal CompAsn6 directories. Assuming that you uploaded the lab files to your CompAsn6 directory, execute the following command:

```
cp /project2/bios11141/CompAsn6/Homo_sapiens_Chr2.fasta ~/CompAsn6/Homo_sapiens_Chr2.fasta
```

With these parts done, we can now submit the job for Midway to run. \span  

To send our job to the compute nodes, we use the `sbatch` command. This command stands for “**q**ueue-**sub**mit,” and its syntax is `sbatch jobsubmitname` (swapping out jobsubmitname for your own job submission script name). 
...
After several minutes, your job should have produced two files: `Homo_sapiens_Chr2_ORFs_forward.csv` and `Homo_sapiens_Chr2_ORFs_reverse.csv`. These files contain the results of our ORF analysis. For Part 4, we will need to copy this data from Midway to our own laptops to perform some basic analysis. For Mac users, this will involve the same `scp` command we used above.<span style="color:purple"> Windows users need to use two programs (PuTTY and WinSCP) to access Midway and to download any files.<span style="color:black"> This is explained at the beginning of this document.
...
<span style="color:blue">
This command combines all the .seq files in examples together and displays the combined data on the terminal window.
</span>

<span style="color:green"> `cat ../examples/*.seq > ./chaetognath.fasta` \span

<span style="color:blue">
This command combines all the .seq files in examples together and saves the combined information to a file called chaetognath.fasta. The `>` indicates that we want to save the concatenated information into a file.
</span>
...
<span style="color:blue">
Assuming the current directory is within the examples folder, the command is "grep 'Toolik Lake' ./shaver_etal.csv > ./toolik.csv"
</span>
...
<span style="color:blue">
grep "Aug" ./shaver_etal.csv | grep "Toolik Lake" > ./toolik_aug.csv
</span>
...
<span style="color:blue">
myUsername="arnavb" <br>
</span>
<span style="color:blue">
ssh $myUsername@midway3.rcc.uchicago.edu
</span>
...
```
cp -r /project2/bios11141/CompAsn6/Images ~/CompAsn6/Images
...
<span style="color:blue">
Assuming the current directory is CompAsn6, the command is "mkdir ./Images2"
</span>
...
<span style="color:green">4.1. Use `scp` to retrieve `human_Chr2_ORFs_forward.csv` and `human_Chr2_ORFs_reverse.csv` from Midway. Provide the command you used below. </span>

<span style="color:blue">
Assuming the current directory is within the BIOS CompAsn 6 local folder, the two commands are "scp arnavb@midway3.rcc.uchicago.edu:/home/arnavb/CompAsn6/human_Chr2_ORFs_forward.csv ./" and "scp arnavb@midway3.rcc.uchicago.edu:/home/arnavb/CompAsn6/human_Chr2_ORFs_reverse.csv ./"
</span>
...
<span style="color:green">4.3. How many ORFs with a length of at **least 450 bases** (150 residue proteins) were detected in the forward and reverse directions of Chromosome 2 of the human genome? </span>
...
<span style="color:green">4.4. Chromosome 2 of the Homo sapiens reference genome is currently known to contain about 2100 genes that code for about 5000 proteins. a) Why might the number ORFs you obtained and gene count be different? b) How can more than one protein be encoded by any given gene? Explain.</span>

<span style="color:blue">
a) The number of ORFs and actual gene count may be different due to the fact that not all ORFs may actually code for functional proteins. Some ORFs may also be pseudogenes, which are non-functional copies of genes that have accumulates mutations and can no longer code for functional proteins. <br>
</span>

<span style="color:blue">
b) More than one protein can be encoded by a given gene through alternative splicing (fusing different combinations of the exons together), or other post-translational modifications. 
</span>

### Part 5: Putting it all together
Let's now put together all of the different skills you learned in this lab to run findORF on more chromosomes.

I wrote a new R script and job-submit script that runs findORF on 4 human chromosomes.  It's called `CompAsn6source-updated.R` and `job-submit-updated.sh`.  There are also the FASTAs for human chromosomes 1-4 in the class directory. These are called `Homo_sapiens_Chr1.fasta`, `Homo_sapiens_Chr2.fasta`, `Homo_sapiens_Chr3.fasta`, and `Homo_sapiens_Chr4.fasta`, respectively.

<span style="color:green">5.1. Copy over the new R script, job-submit script, and the fastas for chromosomes 1-4 from our class directory. Paste your commands in the answer file. </span>

<span style="color:blue">
The new R scripts, and job-submit script is already copied to my personal CompAsn6 folder, so I only need to copy the fasta files. Assuming the current directory is my personal CompAsn6 folder, the command I used is: <br>
</span>

<span style="color:blue">
scp arnavb@midway3.rcc.uchicago.edu:/project2/bios11141/CompAsn6/Homo_sapiens_Chr*.fasta ./
</span>

In your CompAsn6 directory, you should now have the fasta files for human chromosomes 1-4, as well as a new R script called `CompAsn6source-updated.R`. 

<span style="color:green">5.2. View what is inside `CompAsn6source-updated.R`.  Note that this is R code!  What is `CompAsn6source-updated.R` doing and how is it able to run on several chromosomes? (Hint: take a look at the original `CompAsn6source.R` to see how they're different.</span>

<span style="color:blue">
The new R file is able to run on multiple different chromosomes because it takes the fasta file to run on as an input to the code (it uses the sequence that was passed in through the job-submit-updated file)
</span>

Okay, now that we have all these parts, we're almost ready to submit your job!

<span style="color:green">5.3. Right now, `job-submit-updated.sh` is set to run on CompAsn6source.R and only runs on chromosome 2. Change `job-submit-updated.sh` to now run `CompAsn6source-updated.R`.  In addition, add a For Loop to `job-submit-updated.sh` that iteratively runs CompAsn6source-updated.R on chromosomes 1 to 4. Paste your new job submit script in the answer file. </span>
...
<span>
<span style="color:blue">
Rscript $Rinfile $i <br>
<span>
<span style="color:blue">
done
</span>

<span style="color:green">5.4. Submit the `job-submit-updated.sh` and report the number of ORFs found in chromosomes 1 to 4.
...
Read the Midway Files
```{r}
## FIND THE INTERESTING MUTATIONS FOR FURTHER ANALYSIS


mydata <- read.csv("SRR711361.annovar.hg38_multianno.exonic.nonsynonymous.txt", header=TRUE, sep="\t")

mydata_filtered <- mydata[mydata$CLINSIG != "",]

mydata_filtered
```

```{r}
mydata <- read.csv("SRR711361.annovar.hg38_multianno.txt", header=TRUE, sep="\t")


barplot(table(mydata$Func.refGene), main="Distribution of Types of Mutations", xlab="Mutation Type", ylab="Count")
```
```{r}
tab <- table(mydata$Func.refGene)
tab
```
