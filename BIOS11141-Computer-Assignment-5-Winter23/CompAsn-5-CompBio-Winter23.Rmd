...
<span style="color:red"> Please write in code chunks or blue text the answers to all questions that are in green in this .Rmd file. Upload a knitted html file titled "Asn4LastnameFirstname.html" and the Rmd to Canvas by **2:30pm CST the Friday after discussion.**\span

This assignment continues with the subject of sequence alignment, moving from global alignment to local alignment. With local alignment, instead of finding the best alignment between the entirety of two sequences, one finds an alignment with the best score among all pairings of subsequences of the two sequences being aligned. After local alignment, the assignment proceeds to the subject of multiple sequence alignments (aligning more than 2 sequences). Finally, you will learn how to implement the center star algorithm for multiple sequence alignment.
...
<span style="color:green">1.2 What is the alignment matrix that Smith-Waterman generates for the sequences AGCCTATGGCTTATGATCCA and TTAGACAGGGCTTAAGCT? \span
...
The score of the best local alignment is 29.
</span>
...
The following function is modified from [A little book-of R for](https://a-little-book-of-r-for-bioinformatics.readthedocs.io/en/latest/src/chapter4.html), and it generates X number of randomized sequences with the same length and amino acid composition (on average) as the inputted sequence. 
...
<span style="color:green"> 2.1 Similar as you did in the last week's assignment **globally** two sequences MVWHDVDEGFHTAVW and AHDVGVWGHDD using the pairAlign(), `substitutionMatrix="BLOSUM62"`, `gapOpening = -12` and `gapExtension = -3`. What is the alignment and its score?

```{r}
pairAlign("MVWHDVDEGFHTAVW", "AHDVGVWGHDD", substitutionMatrix="BLOSUM62", gapOpening=-12, gapExtension=-3, type="local")
```
<span style="color:blue">
The alignment is clearly shown above and the score of the alignment is 18.
</span>

<span style="color:green"> 2.2 Generate 2000 permuted sequences of the amino acid sequence MVWHDVDEGFHTAVW using generateSeqs(). Then in a for loop, iterativley align "HDVGVWGHDD" to each of these permuted sequences with `substitutionMatrix="BLOSUM62"`, `gapOpening = -10` and `gapExtension = -2`, and save the alignment scores to a vector (Hint: we only want to store the alignment *scores*; `scoreOnly = FALSE` by default). \span
...
<span style="color:green"> 2.3 Create a histogram of your permutations' alignment scores. Calculate the probability of observing a score greater than or equal to what you obtained in Q1.4 from your permutation analysis, and comment on the evidence for evolutionary relatedness of these two sequences (Hint: you want to find the proportion of scores greater than or equal to the actual alignment score you obtained above out of all of the sequence scores.) \span
...
```
<span style="color:blue">
The probability of obtaining a score greater than or equal to the score obtained in 2.1 is around 0.4. Since there is such a high probability it is quite likely that the two sequences are not evolutionary related.
</span>

<span style="color:green"> 2.4 Comment on the probability you calculated? Can you confidently claim that the two sequences are related? Explain your answer.

<span style="color:blue">
No, you can not claim that the two sequences are related. In order to make this claim, the probability due to random chance that the score is greater than or equal to the score obtained in 2.1 should be <= 0.05.
</span>
...
a) Describe how the `buildMSA()` function work (provide details). <br>

<span style="color:blue">
The buildMSA function works by taking in two lists, a list of patterns and list of subjects. First, the first two rows of the matrix are produced from the first aligned pair. The first row is the center sequence after first alignment, and the second row is the other sequence after alignment. Every time you add a new row, if patterns[i] matches the first row of the matrix without a gap, we add the subject to the next row. If patterns[i] has a gap and the first row doesn't, we add a gap to every previous row and then add the subject to the next row. If the first row has a gap that patterns[i] does not, we insert a gap where necessary into subjects[i] and add it into the next row.  
</span>
...
<span style="color:blue">
If this condition is reached, the function will add gaps where necessary in the previous sequences up to this point.
</span>
...
As we can see from this output, the matching occurred much as described above. We first consider Ronda, because Ronda is the first person listed in our vector of names. Ronda shakes hands with every other person, then we move on to consider Eustace. He has already shaken hands with Ronda, so Eustace begins shaking hands with Benedigo, etc. When we reach Philomena, she only has Caecilius left to shake hands with. Note that the iteration does not consider Caecilius because there is no need. All guests have shaken hands by the time he would have been considered.
...
<span style="color:blue">
The first for loop loops from the first index to the second to last index, and the second (inner) for loop loops from the i+1 up until the last index. This enforces that the j index is always larger than the i index. Therefore, the two loops will avoid double counting as the second index is always the larger of the two.
</span>
...
<span style="color:blue">
The center sequence is the sequence that has the minimum column sum from the distance matrix. This is because the center sequence is the sequence that minimizes the sum of the distance from each of the other sequences after sequence alignment.
</span>
...
<span style="color:green"> 3.6 For our list of sequences, find the center sequence and show the steps(codes) you took to get there. </span>
```{r}
possibleCenters <- which(colSums(distanceMatrix) == min(colSums(distanceMatrix)))
...
<span style="color:blue">**We will continue working on the CenterStar algorithm next week by consolidating the above parts into a function `CenterStar()` that takes in a list of sequences and uses `buildup()` to output the multiple sequence alignment.**</span>
