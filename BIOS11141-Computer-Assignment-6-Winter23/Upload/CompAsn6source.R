...
humanChr2 <- seqinr::read.fasta("Homo_sapiens_Chr2.fasta")[[1]]
print(paste0("Chromosome sequence successfully loaded | ", date()))

write.csv(x = ORFTable(humanChr2, 100), file = "./human_Chr2_ORFs_forward.csv", row.names = F)
print(paste0("Forward ORFs written out | ", date()))

write.csv(x = ORFTable(compStrand(humanChr2), 100), file = "./human_Chr2_ORFs_reverse.csv", row.names = F)
